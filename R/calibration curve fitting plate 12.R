...
df <- read_excel("../excel files/apilimod bafilomycin plate 12/DD_Cali_012.xlsx")
...
byWell$pH <- c(6.35, 7, 5.2, 5.5, 4.75, 4.9, 2.65, 3.45, 4.09, 4.3, 4.5, "4.5HC")
...
#plot curve from plate 10 on same graph
byWell10 <- read.csv("../excel files/apilimod bafilomycin plate 10/byWell.csv")
highOtoR10 = byWell10[[which(byWell9$pH == "4.5HC"), "O...R"]]
LC10 <- subset(byWell10, pH != "4.5HC")
LC10$pH <- as.numeric(LC10$pH)

points(LC10$pH, LC10$G...R, col = "blue")
y10 <- 0.937991928 + (0.095637796 - 0.937991928)/(1 + exp((xNew - 4.591590271)/0.555829694))
lines(xNew, y10, col = "blue")

#############write files
write.csv(byWell, "../excel files/apilimod bafilomycin plate 12/byWell.csv")
write.csv(coef(summary(fitmodel)), "../excel files/apilimod bafilomycin plate 12/coefficients.csv")
