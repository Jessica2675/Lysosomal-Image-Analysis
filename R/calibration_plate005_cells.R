...
df <- read_excel("../excel files/apilimod plate 5/DD_Cells_005.xlsx")
df <- df %>% select(c(Plate, Well, FOV, Area, `G / R`, `O / R`, pH, Ca, `log(Ca)`))
...
bywell$Conc <- ifelse(bywell$Well == "B03" | bywell$Well == "C03", "U2OS",
                     ifelse(bywell$Well == "B05" | bywell$Well == "C05", "L4992",
                            ifelse(bywell$Well == "B07" | bywell$Well == "C07", "L6004", "L3200")))


df$Conc <- ifelse(df$Well == "B03" | df$Well == "C03", "none_U2OS",
                      ifelse(df$Well == "B05" | df$Well == "C05", "DMSO_L4992",
                             ifelse(df$Well == "B07" | df$Well == "C07", "DMSO_L6004", "DMSO_L3200")))
...
df$Conc <- factor(df$Conc, levels=c("U2OS", "L4992" ,"L6004","L3200"))


bywell$Conc <- factor(bywell$Conc, levels=c("U2OS", "L4992" ,"L6004","L3200"))

##############save df   ##but first make sure you remove the filter!!!! >:(
df <- df[rowSums(is.na(df)) != ncol(df), ]  #remove rows where all values are na
write.csv(df, "../excel files/apilimod plate 5/df_Cells_005.csv")

#plot
##all data
ggplot(data = df, aes(x = Conc, y = `G / R`)) + 
...
ggplot(data = df, aes(x = Conc, y = `O / R`)) + 
  geom_boxplot(aes(fill = Conc)) +
  ylim(0, 1)
...
count <- ggplot(data = bywell, aes(x = Conc, y = count, colour = Conc)) + 
  geom_point(position = position_dodge(width = 0.5))+ xlab("Cell")
...
count <- ggplot(data = bywell, aes(x = Conc, y = count)) + 
  geom_point(position = position_dodge(width = 0.5), aes(group = Conc, colour = Well))

area <- ggplot(data = bywell, aes(x = Conc, y = Area)) + 
  geom_point(position = position_dodge(width = 0.5), aes(group = Conc, colour = Well))

pH <- ggplot(data = bywell, aes(x = Conc, y = pH)) + 
  geom_point(position = position_dodge(width = 0.5), aes(group = Conc, colour = Well))

Ca <- ggplot(data = bywell, aes(x = Conc, y = Ca)) + 
  geom_point(position = position_dodge(width = 0.5), aes(group = Conc, colour = Well))
