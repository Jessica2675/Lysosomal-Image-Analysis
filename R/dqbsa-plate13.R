...
df <- read.csv("../excel files/apilimod bafilomycin plate 13/DQBSA_Results.csv")
...
df <- df %>% 
  mutate(condition = str_replace(condition, "withdapi", ""))
...
excludedcondition <- exclude %>%
  ...
  inner_join( 
    exclude %>%
      group_by(Time, condition) %>%
      summarise_at(c("Area", "Mean", "IntDen", "RawIntDen"), list(mean)))
write.csv(exclude, "../excel files/apilimod bafilomycin plate 13/excluded_dqbsa13.csv")
