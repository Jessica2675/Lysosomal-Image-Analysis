...
lowGtoR = 0.155266908
highGtoR = 0.678291038
highOtoR = 1.009826
pKa = 4.34992211
slope = 0.354894107


df <- read_excel("../excel files/apilimod plate 9/DD_Cells_009.xlsx")
df <- df %>% select(c(Plate, Well, FOV, Area, `G / R`, `O / R`,`log(Ca)`,pH, Ca))

#calculate pH w/3.85 in calibration curve
df$pH3.85 <- pKa + slope*log((lowGtoR - df$`G / R`)/(df$`G / R` - highGtoR))

#calculate log(Ca) and Ca w/ 3.85 in calibration curve
df$Kd <- 1.03 + 5.4*10^12 * exp(-df$pH3.85/0.189) + 3.11*10^6 * exp(-df$pH3.85/0.412)
df$lowOtoR <- highOtoR/(4.24 + 0.12*exp(0.5*df$pH3.85))
df$logCa3.85 <- log10(df$Kd*(df$`O / R` - df$lowOtoR)/(highOtoR - df$`O / R`))

df$Ca3.85 <- 10^(df$logCa3.85)
...
bywell$Conc <- ifelse(bywell$Well == "B02" | bywell$Well == "C02" | bywell$Well == "D02" , "DMSO_L10900",
                             ifelse(bywell$Well == "B03" | bywell$Well == "C03" | bywell$Well == "D03" ,"100uM_L10900", 
                                    ifelse(bywell$Well == "B04" | bywell$Well == "C04" | bywell$Well == "D04","DMSO_L7364",
                                           ifelse(bywell$Well == "B05" | bywell$Well == "C05" | bywell$Well == "D05","100uM_L7364",
                                                  ifelse(bywell$Well == "B06" | bywell$Well == "C06","DMSO_L2572",
                                                         ifelse(bywell$Well == "B07" | bywell$Well == "C07", "100uM_L2572", "none_U2OS"))))))

df$Conc <- ifelse(df$Well == "B02" | df$Well == "C02" | df$Well == "D02" , "DMSO_L10900",
                  ifelse(df$Well == "B03" | df$Well == "C03" | df$Well == "D03" ,"100uM_L10900", 
                         ifelse(df$Well == "B04" | df$Well == "C04" | df$Well == "D04","DMSO_L7364",
                                ifelse(df$Well == "B05" | df$Well == "C05" | df$Well == "D05","100uM_L7364",
                                       ifelse(df$Well == "B06" | df$Well == "C06","DMSO_L2572",
                                              ifelse(df$Well == "B07" | df$Well == "C07", "100uM_L2572", "none_U2OS"))))))
...
df$cell_line <- factor(df$cell_line, levels=c("U2OS", "L10900", "L2572", "L7364"))

bywell$Conc <- factor(bywell$Conc, levels=c("none", "DMSO", "100uM"))
bywell$cell_line <- factor(bywell$cell_line, levels=c("U2OS", "L10900", "L2572", "L7364"))


##############save df   ##but first make sure you remove the filter!!!! >:(
write.csv(df, "../excel files/apilimod plate 9/df_Cells_009.csv")
