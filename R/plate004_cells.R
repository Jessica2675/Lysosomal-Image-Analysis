...
df <- read_excel("../excel files/apilimod plate 4/DD_well c04 dmso L4992_004.xlsx") 
df <- df %>% rbind(read_excel("../excel files/apilimod plate 4/DD_U2OS_004.xlsx"))
...
bywell$Conc <- ifelse(bywell$Well == "C04","DMSO_L4992", "none_U2OS")
df$Conc <- ifelse(df$Well == "C04","DMSO_L4992", "none_U2OS")
...
write.csv(df, "../excel files/apilimod plate 4/df_Cells_004.csv")

#plot
##all data
ggplot(data = df, aes(x = Well, y = `G / R`)) + 
  geom_boxplot() +
  ylim(0, 1)
